test_that("create_tidytableone routes to checkbox path (strata present)", {
  df <- tibble::tibble(
...
test_that("create_tidytableone routes to no-strata checkbox path", {
  df <- tibble::tibble(
  ...
  tab <- create_tidytableone(df, vars = c("y","race___1"), checkbox = cb)
  expect_true(any(tab$class == "checkbox"))
