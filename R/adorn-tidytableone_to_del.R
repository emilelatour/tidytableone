...
#'   
#'   
#'   # p-value (by var) — headers ------------------------------------------
#'   # 1) Base grouped p for non‑checkbox vars (use Overall row)
#'   p_grouped_base <- tidy_t1 %>%
#'     dplyr::filter(.data$strata == "Overall") %>%
#'     dplyr::transmute(
#'       var      = as.character(.data$var),
#'       class    = .data$class %||% NA_character_,
#'       var_type = .data$var_type %||% NA_character_,
#'       p_value_grouped = dplyr::case_when(
#'         .data$var_type == "continuous" ~ dplyr::coalesce(
#'           .data$oneway_test_equal_var,
#'           .data$oneway_test_unequal_var,
#'           .data$kruskal_test
#'         ),
#'         .data$var_type == "categorical" & (.data$class %||% "") != "checkbox" ~
#'           dplyr::coalesce(.data$chisq_test, .data$fisher_test),
#'         TRUE ~ NA_real_
#'       )
#'     ) %>%
#'     dplyr::distinct(var, .keep_all = TRUE)
#'   
#'   # 2) For checkbox blocks: header p = p("Any selected")
#'   p_checkbox_header <- tidy_t1 %>%
#'     dplyr::filter((.data$class %||% "") == "checkbox") %>%
#'     dplyr::transmute(
#'       var       = as.character(.data$var),
#'       level_var = .data$level_var,
#'       p_level   = .data$p_value_level
#'     ) %>%
#'     dplyr::group_by(.data$var) %>%
#'     dplyr::mutate(
#'       p_level_adj = if (identical(cb_padj_method, "none")) .data$p_level
#'       else stats::p.adjust(.data$p_level, method = cb_padj_method)
#'     ) %>%
#'     dplyr::ungroup() %>%
#'     dplyr::filter(is.na(.data$level_var)) %>%
#'     dplyr::transmute(var, p_checkbox_header = .data$p_level_adj) %>%
#'     dplyr::distinct(var, .keep_all = TRUE)
#'   
#'   # 3) Merge: prefer checkbox header p when available
#'   p_grouped <- p_grouped_base %>%
#'     dplyr::left_join(p_checkbox_header, by = "var") %>%
#'     dplyr::mutate(
#'       p_value_grouped = dplyr::coalesce(.data$p_checkbox_header, .data$p_value_grouped)
#'     ) %>%
#'     dplyr::select(-dplyr::any_of("p_checkbox_header"))
#'   
#'   # 4) Fallback: any available per-row p if grouped p is missing
#'   p_all <- tidy_t1 %>%
#'     dplyr::select(
#'       .data$var,
#'       p_value = dplyr::coalesce(
#'         .data$p_value_level,
#'         .data$chisq_test, .data$fisher_test,
#'         .data$oneway_test_equal_var, .data$oneway_test_unequal_var, .data$kruskal_test
#'       )
#'     ) %>%
#'     dplyr::filter(!is.na(.data$p_value)) %>%
#'     dplyr::distinct()
#'   
#'   # Final numeric p per var for headers
#'   p_i <- p_grouped %>%
#'     dplyr::left_join(p_all, by = "var") %>%
#'     dplyr::transmute(
#'       var,
#'       p_value = dplyr::coalesce(.data$p_value_grouped, .data$p_value)
#'     )
#'   
#'   
#'   #### Create per‑level checkbox p’s for the body rows -------------------------------- 
#'   
#'   # Per-level p-values for checkbox rows (to print on each level line)
...
#'     dplyr::group_by(.data$var) %>%
#'     dplyr::mutate(
#'       p_adj = if (identical(cb_padj_method, "none")) p_raw
#'       else stats::p.adjust(p_raw, method = cb_padj_method)
#'     ) %>%
#'     dplyr::ungroup() %>%
#'     dplyr::transmute(
#'       var, level,
#'       p_value = scales::pvalue(p_adj, accuracy = p_accuracy, decimal.mark = ".", add_p = FALSE)
#'     )
#'   
#'   # Final numeric p per var for headers (fallback to any available per-row p)
#'   p_all <- tidy_t1 %>%
#'     dplyr::select(
#'       .data$var,
#'       p_value = dplyr::coalesce(
...
#'     dplyr::distinct(var, .keep_all = TRUE)
#'   
#'   # Use our header p’s (p_i) to override p_value in tab_pvals (formatted)
#'   p_i_fmt <- p_i %>%
#'     dplyr::mutate(p_value = scales::pvalue(p_value, accuracy = p_accuracy, decimal.mark = ".", add_p = FALSE))
#'   
#'   tab_pvals <- tab_pvals %>%
#'     dplyr::left_join(p_i_fmt, by = "var", suffix = c("", "_hdr")) %>%
#'     dplyr::mutate(p_value = dplyr::coalesce(.data$p_value_hdr, .data$p_value)) %>%
#'     dplyr::select(-dplyr::any_of("p_value_hdr")) %>%
#'     dplyr::distinct(var, .keep_all = TRUE)
#'   
#'   
#'   
...
#'                                                      show_smd = show_smd,
#'                                                      tab_smd = tab_smd,
#'                                                      tab_pvals_cb = tab_pvals_cb)) |>
#'     dplyr::mutate(dplyr::across(.cols = dplyr::everything(),
#'                                 .fns = ~ dplyr::if_else(is.na(.), "", .)))
#'   
#'   #### De-dup checkbox headers & blank header p -------------------------
#'   # add class to the assembled table
#'   adorned_tidy_t1 <- adorned_tidy_t1 |>
#'     dplyr::left_join(var_class, by = "var")
#'   
#'   # mark header rows (they’re the ones where `level` is blank/NA)
#'   adorned_tidy_t1 <- adorned_tidy_t1 |>
#'     dplyr::mutate(
#'       is_header = (is.na(.data$level) | .data$level == "")
#'     )
#'   
#'   # For checkbox blocks:
#'   #  - keep only the FIRST header row
#'   #  - blank the header p_value (per-level rows already carry their own p’s)
#'   adorned_tidy_t1 <- adorned_tidy_t1 |>
#'     dplyr::group_by(.data$var) |>
#'     dplyr::filter(
#'       # non-checkbox: keep everything
#'       is.na(.data$class) | .data$class != "checkbox" |
#'         # checkbox: keep all non-headers, and only the first header
#'         (.data$class == "checkbox" & (!.data$is_header | dplyr::row_number() == 1))
#'     ) |>
#'     dplyr::ungroup() |>
#'     dplyr::mutate(
#'       p_value = dplyr::if_else(.data$class == "checkbox" & .data$is_header, "", .data$p_value)
#'     )
#'   
#'   
#'   #### Apply labels --------------------------------
...
#'   }
#'   
#'     #### Fix: move any stray p-values from checkbox headers down to their first level row ----
#'   # After combining columns, header rows for categorical/checkbox vars end with two spaces
#'   # (because `level` is empty and we glued "{var}  {level}").
#'   adorned_tidy_t1 <- adorned_tidy_t1 |>
#'     # bring class back so we know which blocks are checkboxes
#'     dplyr::left_join(var_class, by = "var") |>
#'     dplyr::mutate(
#'       is_header_disp = stringr::str_detect(var, "\\s\\s$")  # ends with two spaces => header display row
#'     ) |>
#'     dplyr::group_by(var) |>
#'     dplyr::mutate(
#'       # carry header p down to the next row *within the same var* only for checkbox blocks
#'       p_value = dplyr::if_else(
#'         class == "checkbox" & !is_header_disp & dplyr::lag(is_header_disp, default = FALSE) & p_value == "",
#'         dplyr::lag(p_value),
#'         p_value
#'       ),
#'       # blank the header’s p-value for checkbox blocks
#'       p_value = dplyr::if_else(
#'         class == "checkbox" & is_header_disp,
#'         "",
#'         p_value
#'       )
#'     ) |>
#'     dplyr::ungroup() |>
#'     dplyr::select(-class, -is_header_disp)
#'   
#'   #### Top row (n) --------------------------------
#'   
#'   if (any(tidy_t1$var_type == "continuous") & any(tidy_t1$var_type == "categorical")) {
...
#'   
#'   # --- header (per-variable) p-value: force scalar ---
#'   p_i <- tab_pvals %>%
#'     dplyr::filter(.data$var == tab_var) %>%
#'     dplyr::distinct(.data$var, .keep_all = TRUE) %>%
#'     dplyr::pull(.data$p_value) %>%
#'     dplyr::first()
#'   
#'   # If this var is a checkbox block, blank the header p (per-level rows will show p's)
#'   if (!is.null(tab_pvals_cb) && any(tab_pvals_cb$var == tab_var)) {
#'     p_i <- ""
#'   }
#'   
#'   # --- stats rows to display (keep var through the join) ---
#'   s_i <- tab_stats %>%
#'     dplyr::filter(.data$var == tab_var) %>%
#'     dplyr::select(-.data$var_type)  # keep `var` column for the join
#'   
#'   # --- attach per-level p-values for checkbox rows (join by var + level) ---
#'   if (!is.null(tab_pvals_cb) && nrow(tab_pvals_cb)) {
#'     s_i <- s_i %>%
#'       dplyr::left_join(
#'         tab_pvals_cb %>%
#'           dplyr::filter(.data$var == tab_var) %>%
#'           dplyr::select(.data$var, .data$level, .data$p_value),
#'         by = c("var", "level"),
#'         suffix = c("", "")
#'       )
#'   }
#'   
#'   # After joining, hide var in the display rows and ensure a p_value column exists
#'   if (!"p_value" %in% names(s_i)) s_i$p_value <- NA_character_
#'   s_i <- s_i %>%
#'     dplyr::mutate(
#'       var = NA_character_,
#'       p_value = as.character(p_value)  # keep type consistent downstream
#'     )
#'   
#'   # --- test and SMD for the header row ---
#'   t_i <- tab_pvals %>%
#'     dplyr::filter(.data$var == tab_var) %>%
...
#'   }
#'   
#'   if (!show_test) res <- dplyr::select(res, -test)
#'   if (!show_smd)  res <- dplyr::select(res, -smd)
#'   
#'   res
#' }
#' 
