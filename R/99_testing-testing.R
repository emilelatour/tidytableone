...
# dist_mat <- stringdistmatrix(df$var, df$var, method = "jw") # Jaro-Winkler good for prefixes
# 
# # Cluster
# hc <- hclust(as.dist(dist_mat))
# 
# # Cut tree into groups — adjust 'h' for sensitivity
# df <- df %>%
#   mutate(group = cutree(hc, h = 0.3))
# 
# 
# df <- df %>%
#   group_by(group) %>%
#   mutate(group_label = first(var)) %>%
#   ungroup()
# 
# df
# 
